...
router.post('/', [
  ...
  try {
    ...
    // Update teacher's assignedClass field
...
router.get('/:id', [auth, requireSuperAdmin], async (req, res) => {
  try {
    ...
    const students = await Student.find({ 
      assignedClass: classItem.name,
      isActive: true 
    }).select('name dateOfBirth gender parentName parentEmail parentPhone');
...
// @access  Private (Super Admin)
router.get('/teachers/available', [auth, requireSuperAdmin], async (req, res) => {
  try {
    // Get all teachers
    const allTeachers = await User.find({ 
      role: 'teacher',
      isActive: true 
    }).select('name email phone assignedClass');

    // Get teachers who are already class teachers
    const assignedTeachers = await Class.find({ isActive: true }).distinct('classTeacher');

    // Filter out teachers who are already assigned as class teachers
    const availableTeachers = allTeachers.filter(
      teacher => !assignedTeachers.some(assigned => assigned.toString() === teacher._id.toString())
    );
...
router.get('/stats', [auth, requireSuperAdmin], async (req, res) => {
  try {
    ...
    const inactiveClasses = await Class.countDocuments({ isActive: false });

    // Get total capacity and current enrollment
